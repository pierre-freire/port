    ...
    return (
      <div className="overflow-hidden rounded-lg border border-gray-700 bg-gray-800 shadow-sm">
        ...
        <div className="flex flex-wrap gap-2 border-t border-gray-700 p-6">
          {tags.map((tag) => (
            <span key={tag} className="rounded-full bg-gray-700 px-3 py-1 text-sm font-medium text-gray-300">
              {tag}
            </span>
          ))}
          <a href={projectUrl} className="ml-auto text-sm font-medium text-blue-400 hover:underline">
            View Project
          </a>
        </div>
      </div>
